        ...
        try:
            ...
            while pygame.mixer.music.get_busy():
                if func() == False:  # Check if the external function returns False
                    break
                pygame.time.Clock().tick(10)  # Limit loop execution
...
def TextToSpeech(Text, func=lambda r= None: True):
    ...
    if len(Data) > 4 and len(Text) > 250:
        TTS(" ".join(Text.split(".")[0:2]) + "." +random.choice(responses),func)  # Play the first four sentences
...
if __name__ == "__main__":
    while True:
        # Example usage of the TextToSpeech function
